...
export default function SeekerDashboard() {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        console.log('=== FETCHED NOTIFICATIONS DATA (SEEKER) ===');
  ...
  const markAllNotificationsAsRead = async () => {
    console.log('=== MARK ALL READ FUNCTION CALLED (SEEKER) ===');
    try {
  ...
  const handleNewNotification = (newNotifications) => {
    console.log('=== HANDLING NEW NOTIFICATIONS (SEEKER) ===');
    ...
    console.log('=== END HANDLING NEW NOTIFICATIONS (SEEKER) ===');
  };
            ...
            <div className="p-4 border-b border-white/20">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2 notifications-container">
                  ...
                  {console.log('Rendering mark all read button (SEEKER). Unread count:', unreadCount, 'isMarkingAllAsRead:', isMarkingAllAsRead)}
                  {unreadCount > 0 && (
                    <button
                      onClick={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
                        console.log('=== BUTTON CLICKED (SEEKER) ===');
                    ...
                    <div className="flex items-start justify-between">
                      ...
                      {!notification.is_read && (
                        <button
                          ...
                          className="ml-2 p-2 text-blue-600 hover:text-blue-800 hover:bg-blue-100 rounded-lg transition-all duration-200 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
                          title="Mark as read"
                        >
